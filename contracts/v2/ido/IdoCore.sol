...
contract IdoCore is Ownable {
    ...
    function rateAccount(address account) public view returns (uint256) {
        uint256 time = rewardTime(account);
        uint256 balanceRate = (BalanceOf[account] * 1e18) / TotalSupply;
        return (balanceRate / PERIOD_FINISH) * time;
    }
    ...
    function getLPToken(address account) public view returns (uint256) {
        return ((BalanceOf[account] * 1e18) / TotalSupply) * TotalLP;
    }

    function getRewardLP(address account) public view returns (uint256) {
        return (rateAccount(account) * TotalLP) / 1e18;
    }

    function getReward() external OnReward {
        uint256 time = rewardTime(msg.sender);
        uint256 rate = rateAccount(msg.sender);

        uint256 liquidity = (rate * TotalLP) / 1e18;
        ...
        if (amountStaking > amountAccount) {
            ...
            give = ((amountRewards - (Destroy * time)) * rate) / 1e18;
