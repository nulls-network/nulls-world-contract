...
contract NullsEggManager is IOnlineGame, Ownable {
    ...
    event NewPet( uint batchIndex , uint item , address player , uint v , bytes rv ) ;
    ...
    function _openOne( uint8 index , uint item , address player , bytes memory rv ) internal returns ( uint petid ){
        //random v 
    ...
    function openMultiple(
        ...
        // check sign ..
        IOnlineRouter(Proxy).play(itemId, hv, deadline, vs, rs, ss);
    }
}
